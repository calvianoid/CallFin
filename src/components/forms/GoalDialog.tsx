...
export function GoalDialog({ open, onOpenChange, initial }: GoalDialogProps) {
  ...
  const [target, setTarget] = useState<string>(initial?.target_amount ? String(initial.target_amount) : "");
  const [current, setCurrent] = useState<string>(initial?.current_amount ? String(initial.current_amount) : "0");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{isEdit ? "Edit Goal" : "Tambah Financial Goal"}</DialogTitle>
          <DialogDescription>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              ...
              <Input id="target" type="number" value={target} onChange={(e) => setTarget(e.target.value)} placeholder="10000000" required />
            </div>
            <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button type="submit">{isEdit ? "Simpan Perubahan" : "Tambah Goal"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
