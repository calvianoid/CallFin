...
export function TransferDialog({
  open, onOpenChange, defaultFromId, defaultToId, defaultAmount, fromAI, onSaved,
}: TransferDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      const otherId = defaultToId || wallets.find((w) => w.id !== (defaultFromId || wallets[0]?.id))?.id || "";
      setToId(otherId);
  ...
  const insufficient = fromWallet && num > fromWallet.balance && fromWallet.type !== "credit";
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {fromAI ? <Sparkles className="h-4 w-4 text-primary" /> : <ArrowLeftRight className="h-4 w-4 text-primary" />}
            {fromAI ? "Konfirmasi Transfer" : "Transfer Antar Dompet"}
          </DialogTitle>
          <DialogDescription>
            Pindahkan saldo dari satu dompet ke dompet lainnya. Tidak dihitung sebagai pemasukan maupun pengeluaran.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <SelectContent>
                  {wallets.map((w) => (
                    <SelectItem key={w.id} value={w.id} disabled={w.id === fromId}>
                      <span className="mr-1">{w.icon}</span> {w.name}
                      {w.id === fromId && <span className="text-[10px] ml-1 text-muted-foreground">(asal)</span>}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          {sameWallet && (
            <p className="text-xs text-destructive">Pilih dompet tujuan yang berbeda dari asal.</p>
          )}
          ...
          <div className="space-y-2">
            ...
            />
            {insufficient && (
              <p className="text-xs text-amber-600">⚠ Saldo dompet asal tidak cukup, tapi masih bisa lanjut.</p>
            )}
          </div>
          ...
          {fromWallet && toWallet && num > 0 && !sameWallet && (
            <div className="bg-muted rounded-lg p-3 space-y-1 text-xs">
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">{fromWallet.icon} {fromWallet.name}</span>
                <span className="font-medium tabular-nums">{formatRupiah(fromWallet.balance)} → {formatRupiah(fromWallet.balance - num)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">{toWallet.icon} {toWallet.name}</span>
                <span className="font-medium tabular-nums">{formatRupiah(toWallet.balance)} → {formatRupiah(toWallet.balance + num)}</span>
              </div>
            </div>
          )}
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={!num || !fromId || !toId || !!sameWallet}>
